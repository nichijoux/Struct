...
template<class T>
class BST :virtual public Tree<T>
{
protected:
	//将树清空
...
//将树清空,inline 内联函数
template<class T>
inline void BST<T>::ClearTree(TreeNode<T>* node)
{
	if (node != nullptr)
	{
		ClearTree(node->left);
...
//删除节点的辅助函数(递归实现)
template<class T>
TreeNode<T>* BST<T>::DeleteNode(TreeNode<T>* node, const T& key)
{
	//递归终止条件
...
//得到BST中指定值的节点
template<class T>
TreeNode<T>* BST<T>::GetNode(const T& val)
{
	TreeNode<T>* tempNode = this->root;
